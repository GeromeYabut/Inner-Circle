	...
	for(var i = 0;i < results.length;i++)
		{
			...
			//location
			$('<li>').text("Location:").appendTo('#ul' + i);    
			$('<li id="inline">').text(posts.Location).appendTo('#ul' + i);
			// $('<ul id=' +i +'>').text(posts.Location).appendTo('#matches');
			// //Description
			$('<li>').text("About:").appendTo('#ul' + i);
			 $('<li id="inline">').text(posts.Description).appendTo('#ul' + i);
			// //Date
